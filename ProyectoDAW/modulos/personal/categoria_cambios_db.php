...
<head>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#mensaje').dialog({
                ...
                buttons: {
                    "Modificar Otra Categoria": function () {
                        $("#mensaje").dialog("close");
                        location      = 'categoria_cambios.php';
                    },
    ...
    <?php
        ...
        $nombre = validar($_POST['nuevo_nombre']);
    ?>
</head>    
<body>
    <?php
        try {
            //Crear la consulta
            $sql = "update Sakila.category
                    set name   = :nombre
                    where category_id = :id";

            $cambios = $conexion->prepare($sql);
            $cambios->bindParam(':nombre' , $nombre);
    ...
    ?>
            <script>
                $(document).ready(function () {
                    $("#mensaje").html("<h1>CATEGORIA MODIFICADA CON EXITO</h1>");
                    $("#mensaje").dialog("open");    
                });
            </script>
    <?php
